...
class ShowTimeSimulation extends Simulation {
  ...
  .exec(http("layout")
     ...
     .body(StringBody("""{"sessionId":"${session_id}","quantity":"2","seatCategory":"ELITE","orderId":"${orderId}","isAutoSelected":false}""")).asJSON
     .check(status.is(200)))

  setUp(scn.inject(ramp(2000 users) over (100 seconds))).protocols(httpConf)
}