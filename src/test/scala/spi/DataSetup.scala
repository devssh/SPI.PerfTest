...
object DataSetup {
  ...
  val sessionsQuery =
    ...
    "sessions.id in (select id from sessions where date(start_time) = CURRENT_DATE+8 and cinema_name = 'Sathyam Cinemas' limit 20) or " +
    "sessions.id in (select id from sessions where date(start_time) = CURRENT_DATE+8 and cinema_name = 'LUXE' limit 20) or " +
    "sessions.id in (select id from sessions where date(start_time) = CURRENT_DATE+8 and cinema_name = 'Escape' limit 20) "
